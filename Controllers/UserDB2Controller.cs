...
namespace WebApplication2022_Core6_6th_RelationalDB.Controllers
{
    public class UserDB2Controller : Controller
    {
        ...
        #region
        //private readonly MVC_UserDB2Context _db = new MVC_UserDB2Context();  // .NET 4.x版的寫法。
        private readonly MVC_UserDB2Context _db;  // 新版 .NET Core使用DI（依賴性注入）所以不像以前還要加寫= new MVC_UserDB2Context();
        ...
        {   //  無須使用 ViewModel，直接完成。
            ...
            //******************************************
            //（錯誤）  
            //return View(_db.UserTable2s);  // 以前 .NET 4.x版MVC、EF的作法，在.NET Core出問題。透過SQL Profiler觀察，沒有做JOIN。
            // .NET Core 6 必須改寫如下：
        ...
        {   ////方法一：
            ...
            // 以前的 .NET 4.x MVC、EF的作法，在.NET Core會出現問題。透過SQL Profiler觀察，沒有做JOIN。
            // 必須改寫如下：*******************************************************************
        ...
        {   // 參考資料：使用 .include()   https://msdn.microsoft.com/en-us/library/jj574232(v=vs.113).aspx
            ...
            // SQL Profile翻譯後的成果 ---- 以前 .NET 4.x 的MVC與EF，翻譯成 Inner Join。現在的 .NET Core / EF Core 翻譯成LEFT JOIN
            #region
            // SELECT[u].[UserId], [u].[DepartmentId], [u].[UserApproved], [u].[UserBirthDay], [u].[UserMobilePhone], [u].[UserName], [u].[UserSex], [d].[DepartmentId], [d].[DepartmentName], [d].[DepartmentYear]
            // FROM[UserTable2] AS[u]
            // LEFT JOIN[DepartmentTable2] AS[d] ON[u].[DepartmentId] = [d].[DepartmentId]
        ...
        {   // 原廠（中文）說明  https://docs.microsoft.com/zh-tw/dotnet/framework/data/adonet/ef/language-reference/query-expression-syntax-examples-navigating-relationships
            ...
            //// 錯誤版本，讓您比對。與上面的差異只在「select new」這一句。
            #region
            //var result = from ut in _db.UserTable2s
            //             where ut.DepartmentId == _ID
            //                               ************
            //             select new UserTable2   //*** 重點！！如果您不使用自己改寫過的 ViewModel，直接使用 UserTable2類別，
            //             {            // ************* 執行檢視畫面時，.NET 4.x版就會出錯 --「LINQ to Entities 查詢中無法建構實體或複雜類型 'WebApplication2017_MVC_GuestBook.Models2.UserTable2'。」
            //                 // .NET Core 6的錯誤訊息：
            //                 // InvalidOperationException: The model item passed into the ViewDataDictionary is of type 'Microsoft.EntityFrameworkCore.Query.Internal.EntityQueryable`1[WebApplication2022_Core6_6th_RelationalDB.Models2.UserTable2]', but this ViewDataDictionary instance requires a model item of type 'System.Collections.Generic.IEnumerable`1[WebApplication2022_Core6_6th_RelationalDB.Models2.UserTable2ViewModel]'.
            //
            //                 UserId = ut.UserId,
            //                 UserName = ut.UserName,
            //                 UserSex = ut.UserSex,
        ...
        {    
           ...
           var GroupJoinMS = _db.DepartmentTable2s.AsEnumerable()  //Outer Data Source （科系）
